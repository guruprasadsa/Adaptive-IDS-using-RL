...
export function renderRLModelPage(
    ...
    pageHeader.innerHTML = `
        ...
        <div>
            <h1 class="page-title">RL Model Dashboard</h1>
            <p class="page-subtitle">Monitor reinforcement learning model performance and metrics in real-time</p>
        </div>
    `;
    ...
    if (!metrics) {
        const loadingSpinner = createLoadingSpinner({
            size: 'lg',
            text: 'Loading model metrics...',
            center: true
        });
        mainCard.appendChild(loadingSpinner);
    } else {
        // Performance Metrics Section
        const metricsSection = document.createElement('div');
        metricsSection.className = 'metrics-section mb-8';
        
        const metricsHeader = document.createElement('div');
        metricsHeader.className = 'section-header';
        ...
        // Create metric cards
        const metricsData = [
            { label: 'Accuracy', value: `${(metrics.accuracy * 100).toFixed(2)}%`, icon: 'radio_button_checked', colorClass: 'metric-success' },
            { label: 'Precision', value: `${(metrics.precision_macro * 100).toFixed(2)}%`, icon: 'center_focus_strong', colorClass: 'metric-info' },
            { label: 'Recall', value: `${(metrics.recall_macro * 100).toFixed(2)}%`, icon: 'search', colorClass: 'metric-warning' },
            { label: 'F1 Score', value: `${(metrics.f1_macro * 100).toFixed(2)}%`, icon: 'star', colorClass: 'metric-warning' },
            { label: 'ROC AUC', value: `${(metrics.roc_auc * 100).toFixed(2)}%`, icon: 'trending_up', colorClass: 'metric-success' },
            { label: 'Balanced Accuracy', value: `${(metrics.balanced_accuracy * 100).toFixed(2)}%`, icon: 'balance', colorClass: 'metric-danger' },
        ];
        ...
        const confusionSection = document.createElement('div');
        confusionSection.className = 'metrics-section';
        
        const confusionHeader = document.createElement('div');
        confusionHeader.className = 'section-header';
        confusionHeader.innerHTML = `
            <span class="material-symbols-outlined">grid_on</span>
            <h3>Confusion Matrix Statistics</h3>
        `;
        
        const confusionGrid = document.createElement('div');
        confusionGrid.className = 'metrics-grid';
        
        const confusionData = [
            { label: 'Total Predictions', value: metrics.total_predictions.toLocaleString(), icon: 'dataset', colorClass: 'metric-default' },
            { label: 'True Positives', value: metrics.true_positives.toLocaleString(), icon: 'check_circle', colorClass: 'metric-success' },
            { label: 'True Negatives', value: metrics.true_negatives.toLocaleString(), icon: 'shield', colorClass: 'metric-success' },
            { label: 'False Positives', value: metrics.false_positives.toLocaleString(), icon: 'warning', colorClass: 'metric-warning' },
            { label: 'False Negatives', value: metrics.false_negatives.toLocaleString(), icon: 'cancel', colorClass: 'metric-danger' },
        ];
        
        confusionData.forEach(metric => {
            confusionGrid.appendChild(createMetricCard(metric));
        });
        
        confusionSection.appendChild(confusionHeader);
        confusionSection.appendChild(confusionGrid);
        
        mainCard.appendChild(metricsSection);
        mainCard.appendChild(confusionSection);
    }
