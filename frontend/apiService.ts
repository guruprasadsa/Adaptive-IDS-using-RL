...
import apiClient, { cachedGet, clearCache, tokenManager } from './utils/apiClient';
...
export { clearCache, tokenManager };
...
class ApiService {
    ...
    async acknowledgeAlert(id: string, notes?: string): Promise<Alert> {
        const response = await apiClient.patch<Alert>(`/alerts/${id}/ack`, { notes });
        clearCache('alerts'); // Invalidate alerts cache
        return response.data;
    }

    // Mark alert as false positive
    async markAlertFalsePositive(id: string, feedback?: string): Promise<Alert> {
        const response = await apiClient.post<Alert>(`/alerts/${id}/false-positive`, { feedback });
        clearCache('alerts'); // Invalidate alerts cache
        return response.data;
    }
